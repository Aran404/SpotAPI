...
from typing import TYPE_CHECKING, Self, Unpack, ClassVar, Final, Mapping, Sequence, Any
...
class HTTPClient:
    ...
    def get_all_cookies(self) -> list[dict[str, Any]]:
        if not self._wclient.cookie_jar:
            return []
        return [dict(m) for m in self._wclient.cookie_jar.get_all()]  # type: ignore[unknown-return-type]

    def set_cookies(self, cookies: list[Mapping[str, Any]]) -> None:
        urls: Sequence[str] = [
            ...
            "https://dealer.spotify.com",
            "https://gds.spotify.com",
        ...
        for c in cookies:
            for url in urls:
                self._wclient.cookie_jar.add(wreq.Cookie(**c), url)
