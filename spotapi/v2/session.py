...
class AuthSession:
    ...
    async def from_cookies(
        cls: type[AuthSession], cookies: list[Mapping[str, Any]] | str, proxy: str | None = None
    ) -> AuthSession:
        if isinstance(cookies, str):
            parsed_cookies: list[Mapping[str, Any]] = []
            for cookie in cookies.split(";"):
                if "=" in cookie:
                    k, v = cookie.strip().split("=", 1)
                    parsed_cookies.append({"name": k, "value": v})
            cookies = parsed_cookies

        client = await AsyncClient.new(proxy=proxy)
        client.protocols.http.set_cookies(cookies)
    ...
    def authorized_headers(self) -> dict[str, str]:
        if not self.token.client_token:
            raise RuntimeError("Client token not set for authorized headers")
