...
from spotapi.v2.connection import (
    ClientPool,
    HTTPClient,
    Response,
    ResponseSuccess,
...
def _validate(response: Response) -> ResponseSuccess:
    ...
    if not response.status_code.is_success():
        raise HTTPError(f"Request returned non-ok status code: {response.status_code.as_int()}")

    content_type = response.headers.get("content-type") if response.headers else None
    if content_type and "application/json" in str(content_type) and response.json is None:
        raise HTTPError("Expected a JSON payload (application/json), but received an empty body.")
