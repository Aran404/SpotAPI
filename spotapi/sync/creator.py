...
import uuid
import json
from spotapi.sync.client import RECAPTCHA_SITE_KEY
from spotapi.sync.types.annotations import enforce
from spotapi.sync.types import Config
from spotapi.sync.exceptions import GeneratorError
from spotapi.sync.http.request import TLSClient
from spotapi.sync.utils.strings import (
    random_email,
    random_string,
    parse_json_string,
    random_dob,
...
class Creator:
    ...
    def _process_register(self, captcha_token: str) -> None:
        ...
        if resp.fail:
            raise GeneratorError("Could not process registration", error=resp.error.string)
...
class AccountChallenge:
    ...
    def _get_session(self) -> None:
        ...
        if resp.fail:
            raise GeneratorError("Could not get challenge session", error=resp.error.string)
    ...
    def _complete_challenge(self) -> None:
        ...
        if resp.fail:
            raise GeneratorError("Could not complete challenge", error=resp.error.string)
