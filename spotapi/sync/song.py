...
from spotapi.sync.exceptions import SongError
...
from spotapi.sync.playlist import PrivatePlaylist, PublicPlaylist
...
class Song:
    ...
    def __init__(
        ...
        *,
        ...
        self.base = BaseClient(
            client=playlist.login.client if playlist else client, language=language
        )
    ...
    def paginate_songs(self, query: str, /) -> Generator[Mapping[str, Any], None, None]:
        ...
        while offset < total_count:
            yield self.query_songs(query, limit=UPPER_LIMIT, offset=offset)["data"]["searchV2"][
                "tracksV2"
            ]["items"]
            offset += UPPER_LIMIT
    ...
    def parse_playlist_items(
        ...
        *,
        ...
        for item in items:
            ...
            is_song_name = (
                song_name and song_name.lower() in str(item["itemV2"]["data"]["name"]).lower()
            )
