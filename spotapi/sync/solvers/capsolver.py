...
from spotapi.sync.exceptions import CaptchaException, SolverError
from spotapi.sync.http.request import StdClient
...
class Capsolver:
    ...
    def get_balance(self) -> float | None:
        ...
        if request.fail:
            raise CaptchaException("Could not retrieve balance.", error=request.error.string)

        resp = request.response

        if int(resp["errorId"]) != 0:
            raise CaptchaException("Could not retrieve balance.", error=resp["errorDescription"])
    ...
    def _create_task(
        ...
        task_type = (
            "ReCaptcha{}EnterpriseTask" if proxy else "ReCaptcha{}EnterpriseTaskProxyLess"
        ).format(task.upper())
        ...
        if int(resp["errorId"]) != 0:
            raise CaptchaException("Could not create task.", error=resp["errorDescription"])
    ...
    def _harvest_task(self, task_id: str, retries: int) -> str:
        for _ in range(retries):
            ...
            if request.fail:
                raise CaptchaException("Could not get task result", error=request.error.string)
