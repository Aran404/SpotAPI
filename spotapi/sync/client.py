...
from spotapi.sync.utils.logger import Logger
from spotapi.sync.types.annotations import enforce
from spotapi.sync.types.alias import _UStr, _Undefined
from spotapi.sync.exceptions import BaseClientError
from spotapi.sync.http.data import Response
from spotapi.sync.http.request import TLSClient
from spotapi.sync.utils.strings import extract_js_links, extract_mappings, combine_chunks
...
class BaseClient:
    ...
    def _auth_rule(self, kwargs: dict) -> dict:
        ...
        if (
            ...
            and time.time() * 1000 + self._REFRESH_SKEW_MS >= self.access_token_expires_at_ms
        ):
