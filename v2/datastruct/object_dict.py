...
class ObjectDict(dict[Any, Any]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        object.__setattr__(
            self, "_lock", threading.Lock() if lock_attribute_writes else None
        )
        ...
        super().__init__(
            {
                (
                    f"_{k}" if isinstance(k, str) and k in dict.__dict__ else k
                ): self._wrap(v)
    ...
    @classmethod
    def from_json(
        cls: type[ObjectDict], data: _T, *, lock_attribute_writes: bool = ...
    ) -> _T: ...
    ...
    def from_json(
        ...
        *,
        ...
        if isinstance(data, dict):
            return cls(data, lock_attribute_writes=lock_attribute_writes)
        if isinstance(data, list):
            return [
                cls.from_json(item, lock_attribute_writes=lock_attribute_writes)
                for item in data
            ]
    ...
    def _wrap(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            return ObjectDict(value, lock_attribute_writes=self._is_locked())
        return value
