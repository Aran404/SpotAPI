...
import wreq
from v2.connection.types import (
    ResponseFailure,
    ResponseSuccess,
    DefaultOSList,
    DefaultProfileList,
...
from v2.datastruct import Pool, ObjectDict
from v2.utils import random_with_weights
...
ClientPool: Pool[HTTPClient] = Pool(
    factory=lambda: HTTPClient(), teardown=lambda t: t.close()
)
...
class HTTPClient:
    ...
    def __init__(
        ...
        **kwargs: Unpack[wreq.ClientConfig],
        ...
        _default_emulation_profile: wreq.Emulation = (
            self._create_new_emulation_profile()
        )
    ...
    def _compute_delay(self, attempt: int) -> float:
        base: float = (
            self._backoff
            if isinstance(self._backoff, (int, float))
