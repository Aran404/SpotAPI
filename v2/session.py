...
from v2.specialized.totp import build_totp
...
class BundleSession:
    ...
    def _extract_embedded_links(bundles: dict[str, str]) -> list[str]:
        ...
        return [
            BundleSession._CDN_PREFIX + chunk
            for chunk in combine_chunks(name_map, hash_map)
        ]
...
class AuthSession:
    ...
    def token(self) -> AccessToken:
        if self._token is None:
            raise RuntimeError(
                "token is unavailable until authorize() has been awaited"
            )
        return self._token
